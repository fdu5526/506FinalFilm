...
Shader "ACR/Time Of Day Free/Skybox/Procedural"  
{
...
//      ------------------------------------------------ 
		...
		// Sun disk.
		//----------------------------------------------
		//#pragma multi_compile __ SUNDISK_NONE 
		//#pragma multi_compile __ SUNDISK_SIMPLE
		//#pragma multi_compile __ SUNDISK_HIGH_QUALITY
		//______________________________________________

		// Night sky.
		//----------------------------------------------
		#pragma multi_compile __ NIGHTCOLOR
...
//      =================================================================================================================================================================
		...
		uniform half   _NightAtmosphereThickness;
...
//      =================================================================================================================================================================
		...
		//----------------------------------------------------------------------------------------------------------------
		half HorizonFade(float dir, float exponent)
		{
			return pow(max(0, dir), exponent);
		}
		//_________________________________________________________________________________________________________________
		...
		v2f vert (appdata_t v)
		{
			...
			//-----------------------------------------------------------------------------------------------------------------------------
			#if defined(MOON)
			OUT.moonCoords.w   = saturate(dot(_MoonDir.xyz, OUT.worldPos)); // 2 Side matrix fix.
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			// Horizon fade ( experimental )
			//---------------------------------------------------------------------------------------------------------------------------------------
			#if defined (HORIZONFADE)
			half horizonFade = saturate(HorizonFade(ray.y/0.1, _HorizonFade));
			//IN.skyColor     += horizonFade;
			#endif
			//---------------------------------------------------------------------------------------------------------------------------------------
			...
			//---------------------------------------------------------------------------------------------------------------------------------------
			#if defined(MOON)  
			half4 moonColor  = (tex2D(_MoonTexture, IN.moonCoords.xy) * _MoonColor) * IN.moonCoords.w;
			...
			half mie = getMiePhase(-dot(_SunDir.xyz,ray));
